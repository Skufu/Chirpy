...
const getItemsAsc = `-- name: GetItemsAsc :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
ORDER BY created_at ASC
`

func (q *Queries) GetItemsAsc(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getItemsAsc)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getItemsDesc = `-- name: GetItemsDesc :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
ORDER BY created_at DESC
`

func (q *Queries) GetItemsDesc(ctx context.Context) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getItemsDesc)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
